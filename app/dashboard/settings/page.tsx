    ...
    },
    {
      ...
      Icon: () => (
        <svg width="28" height="28" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          ...
          <path d="M11.2 9.5c0-.7.6-1 1.5-1 1.3 0 2.9.4 4.2 1.1V6.4C15.6 5.6 14 5.3 12.4 5.3c-3.1 0-5.2 1.6-5.2 4.4 0 4.3 5.9 3.6 5.9 5.4 0 .8-.7 1.1-1.7 1.1-1.5 0-3.3-.6-4.8-1.4v3.3c1.6.7 3.2 1 4.8 1 3.2 0 5.3-1.6 5.3-4.4C16.7 10.2 11.2 11 11.2 9.5z" fill="white"/>
        </svg>
      ),
      action: { label: stripeEnabled ? "Manage in Stripe" : "Connect Stripe", cls: "border border-gray-200 text-gray-600 hover:bg-gray-50", href: undefined },
    },
    {
      key: "twilio",
      name: "Twilio SMS",
      ...
      action: { label: "View Usage", cls: "border border-gray-200 text-gray-600 hover:bg-gray-50", href: undefined },
    },
  ];

  const COMING_SOON = ["Facebook Reviews", "Yelp", "HomeStars"];
  ...
  return (
    <div className="space-y-4">
      {INTEGRATIONS.map(int => {
        const Icon = int.Icon;
        return (
          <div key={int.key} className="flex items-center gap-4 p-4 border border-gray-200 rounded-xl bg-white">
            ...
            <div className="flex items-center gap-3 shrink-0">
              {int.status === "connected" && (
                <span className="text-xs font-semibold text-[#15803D] bg-[#F0FDF4] px-2.5 py-1 rounded-full">✓ Connected</span>
              )}
              {int.action.href ? (
                <a href={int.action.href}
                  className={`text-xs px-3 py-1.5 rounded-lg font-semibold transition-colors ${int.action.cls}`}>
                  {int.action.label}
                </a>
              ) : (
                <button className={`text-xs px-3 py-1.5 rounded-lg font-semibold transition-colors ${int.action.cls}`}>
                  {int.action.label}
                </button>
              )}
            </div>
      ...
      <div className="pt-2">
        ...
        <div className="flex flex-wrap gap-3">
          {COMING_SOON.map(name => (
            <div key={name} className="flex items-center gap-2 px-4 py-3 border border-gray-100 rounded-xl bg-gray-50 opacity-60">
              <span className="text-sm text-gray-500 font-medium">{name}</span>
              <span className="text-[10px] bg-gray-200 text-gray-500 px-2 py-0.5 rounded-full font-semibold uppercase tracking-wide">Coming soon</span>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="p-8 max-w-2xl">
      ...
      {activeTab === "Integrations" && (
        <div>
          <h2 className="font-semibold text-[#0d1117] mb-1">Integrations</h2>
