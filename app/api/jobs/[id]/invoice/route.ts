...
// "Send Payment Link" — transitions job to awaiting_payment which fires the SMS side-effect
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const updated = await transitionJobStatus(id, "awaiting_payment", {
      triggeredBy: session.user.id,
      note: "Payment link sent manually via dashboard",
    });
