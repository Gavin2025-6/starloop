...
LOGFILE="/Users/zhenyonggao/Desktop/FactoryOS/logs/auto-analyze.log"
...
while true; do
  ...
  " 2>/dev/null || echo "0")
  echo "$TS PENDING: ${PENDING}" >> "$LOGFILE"
  if [ "${PENDING:-0}" -gt 0 ] 2>/dev/null; then
    echo "$TS Analyzing ${PENDING} reviews..." >> "$LOGFILE"
    /usr/local/bin/npx tsx scripts/analyze-reviews.ts --batch 200 --delay 300 >> "$LOGFILE" 2>&1
    echo "$TS Batch done." >> "$LOGFILE"
  fi
  sleep 600
done
